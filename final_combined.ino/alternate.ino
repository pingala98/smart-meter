...
double meter=0;
double cost=0;
long long int millisec=millis();
long prog_start=millisec/1000;    //time prog started
long long int amptime=0; 
...
double total_amps;
double timep=millis()/(millis())-amptime;

total_amps+=amps_rms; // calculate total amps
...
double energy=(power*timep)/(1000*3600);
meter+=energy;
amptime=millis();
cost+=energy*price;
...
/***************WIFI_CODE****************/
send_data(y,"field1");
send_data(amps_rms,"field2");
send_data(power,"field3");
send_data(energy,"field4");
send_data(cost,"field5");
/***************WIFI_CODE****************/
}
...
/***************WIFI_CODE****************/
void send_data(int sendVal,String myFIELD)
  {
